...
namespace WorkdayWebServices.Student_FinanceService
{
	...
	[Serializable]
	public class Get_Student_Historical_Charges_ResponseType : INotifyPropertyChanged
	{
		private Student_Historical_ChargeObjectType[] request_ReferencesField;

		private Student_Historical_Charge_Request_CriteriaType request_CriteriaField;

		private Response_FilterType response_FilterField;

		private Response_ResultsType response_ResultsField;

		private Student_Historical_ChargeType[] response_DataField;
		...
		[XmlArray(Order = 0), XmlArrayItem("Student_Historical_Charge_Reference", IsNullable = false)]
		public Student_Historical_ChargeObjectType[] Request_References
		{
			get
			{
		...
		[XmlElement(Order = 1)]
		public Student_Historical_Charge_Request_CriteriaType Request_Criteria
		{
			get
			{
		...
		[XmlArray(Order = 4), XmlArrayItem("Student_Historical_Charge", IsNullable = false)]
		public Student_Historical_ChargeType[] Response_Data
		{
			get
			{
