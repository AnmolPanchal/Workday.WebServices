...
namespace WorkdayWebServices.Financial_ManagementService
{
	...
	[Serializable]
	public class Get_Suspense_Account_Rules_ResponseType : INotifyPropertyChanged
	{
		private Suspense_Account_RuleObjectType[] request_ReferencesField;

		private Suspense_Account_Rule_Request_CriteriaType request_CriteriaField;

		private Response_FilterType response_FilterField;

		private Response_ResultsType response_ResultsField;

		private Suspense_Account_RuleType[] response_DataField;
		...
		[XmlArray(Order = 0), XmlArrayItem("Suspense_Account_Rule_Reference", IsNullable = false)]
		public Suspense_Account_RuleObjectType[] Request_References
		{
			get
			{
		...
		[XmlElement(Order = 1)]
		public Suspense_Account_Rule_Request_CriteriaType Request_Criteria
		{
			get
			{
		...
		[XmlArray(Order = 4), XmlArrayItem("Suspense_Account_Rule", IsNullable = false)]
		public Suspense_Account_RuleType[] Response_Data
		{
			get
			{
