...
namespace WorkdayWebServices.Cash_ManagementService
{
	...
	[Serializable]
	public class Get_Investment_Pool_Adjustments_ResponseType : INotifyPropertyChanged
	{
		private Investment_Pool_AdjustmentObjectType[] request_ReferencesField;

		private Investment_Pool_Adjustment_Request_CriteriaType request_CriteriaField;

		private Response_FilterType response_FilterField;

		private Investment_Pool_Adjustment_Response_GroupType response_GroupField;

		private Response_ResultsType response_ResultsField;

		private Investment_Pool_AdjustmentType[] response_DataField;
		...
		[XmlArray(Order = 0), XmlArrayItem("Investment_Pool_Adjustment_Reference", IsNullable = false)]
		public Investment_Pool_AdjustmentObjectType[] Request_References
		{
			get
			{
		...
		[XmlElement(Order = 1)]
		public Investment_Pool_Adjustment_Request_CriteriaType Request_Criteria
		{
			get
			{
		...
		[XmlElement(Order = 3)]
		public Investment_Pool_Adjustment_Response_GroupType Response_Group
		{
			get
			{
		...
		[XmlArray(Order = 5), XmlArrayItem("Investment_Pool_Adjustment", IsNullable = false)]
		public Investment_Pool_AdjustmentType[] Response_Data
		{
			get
			{
