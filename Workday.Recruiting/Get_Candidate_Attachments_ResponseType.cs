...
namespace WorkdayWebServices.RecruitingService
{
	...
	[Serializable]
	public class Get_Candidate_Attachments_ResponseType : INotifyPropertyChanged
	{
		private Candidate_Attachment_Request_ReferencesType request_ReferencesField;

		private CandidateObjectType[] request_CriteriaField;

		private Response_FilterType response_FilterField;

		private Candidate_Attachment_Response_GroupType response_GroupField;

		private Response_ResultsType response_ResultsField;

		private Candidate_Attachment_WWSType[] response_DataField;
		...
		[XmlElement(Order = 0)]
		public Candidate_Attachment_Request_ReferencesType Request_References
		{
			get
			{
		...
		[XmlArray(Order = 1), XmlArrayItem("Candidate_Reference", IsNullable = false)]
		public CandidateObjectType[] Request_Criteria
		{
			get
			{
		...
		[XmlElement(Order = 3)]
		public Candidate_Attachment_Response_GroupType Response_Group
		{
			get
			{
		...
		[XmlArray(Order = 5), XmlArrayItem("Candidate_Attachment", IsNullable = false)]
		public Candidate_Attachment_WWSType[] Response_Data
		{
			get
			{
