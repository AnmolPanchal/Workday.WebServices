...
namespace WorkdayWebServices.InventoryService
{
	...
	[Serializable]
	public class Get_Inventory_Count_Sheets_ResponseType : INotifyPropertyChanged
	{
		private Inventory_Count_SheetObjectType[] request_ReferencesField;

		private Inventory_Count_Sheet_Request_CriteriaType request_CriteriaField;

		private Response_FilterType response_FilterField;

		private Inventory_Count_Sheet_Response_GroupType response_GroupField;

		private Response_ResultsType response_ResultsField;

		private Inventory_Count_SheetType[] response_DataField;
		...
		[XmlArray(Order = 0), XmlArrayItem("Inventory_Count_Sheet_Reference", IsNullable = false)]
		public Inventory_Count_SheetObjectType[] Request_References
		{
			get
			{
		...
		[XmlElement(Order = 1)]
		public Inventory_Count_Sheet_Request_CriteriaType Request_Criteria
		{
			get
			{
		...
		[XmlElement(Order = 3)]
		public Inventory_Count_Sheet_Response_GroupType Response_Group
		{
			get
			{
		...
		[XmlArray(Order = 5), XmlArrayItem("Inventory_Count_Sheet", IsNullable = false)]
		public Inventory_Count_SheetType[] Response_Data
		{
			get
			{
