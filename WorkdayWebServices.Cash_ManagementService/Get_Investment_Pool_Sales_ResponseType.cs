...
namespace WorkdayWebServices.Cash_ManagementService
{
	...
	[Serializable]
	public class Get_Investment_Pool_Sales_ResponseType : INotifyPropertyChanged
	{
		private Investment_Pool_SaleObjectType[] request_ReferencesField;

		private Investment_Pool_Sale_Request_CriteriaType request_CriteriaField;

		private Response_FilterType response_FilterField;

		private Investment_Pool_Sale_Response_GroupType response_GroupField;

		private Response_ResultsType response_ResultsField;

		private Investment_Pool_SaleType[] response_DataField;
		...
		[XmlArray(Order = 0), XmlArrayItem("Investment_Pool_Sale_Reference", IsNullable = false)]
		public Investment_Pool_SaleObjectType[] Request_References
		{
			get
			{
		...
		[XmlElement(Order = 1)]
		public Investment_Pool_Sale_Request_CriteriaType Request_Criteria
		{
			get
			{
		...
		[XmlElement(Order = 3)]
		public Investment_Pool_Sale_Response_GroupType Response_Group
		{
			get
			{
		...
		[XmlArray(Order = 5), XmlArrayItem("Investment_Pool_Sale", IsNullable = false)]
		public Investment_Pool_SaleType[] Response_Data
		{
			get
			{
