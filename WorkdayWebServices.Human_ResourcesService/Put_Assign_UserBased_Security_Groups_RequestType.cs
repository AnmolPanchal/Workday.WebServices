using System;
using System.CodeDom.Compiler;
using System.ComponentModel;
using System.Diagnostics;
using System.Runtime.CompilerServices;
using System.Xml.Schema;
using System.Xml.Serialization;

namespace WorkdayWebServices.Human_ResourcesService
{
	[GeneratedCode("System.Xml", "4.6.1590.0"), DesignerCategory("code"), DebuggerStepThrough, XmlType(TypeName = "Put_Assign_User-Based_Security_Groups_RequestType", Namespace = "urn:com.workday/bsvc")]
	[Serializable]
	public class Put_Assign_UserBased_Security_Groups_RequestType : INotifyPropertyChanged
	{
		private Assign_UserBased_Security_Groups_DataType assign_UserBased_Security_Groups_DataField;

		private string versionField;

		[method: CompilerGenerated]
		[CompilerGenerated]
		public event PropertyChangedEventHandler PropertyChanged;

		[XmlElement("Assign_User-Based_Security_Groups_Data", Order = 0)]
		public Assign_UserBased_Security_Groups_DataType Assign_UserBased_Security_Groups_Data
		{
			get
			{
				return this.assign_UserBased_Security_Groups_DataField;
			}
			set
			{
				this.assign_UserBased_Security_Groups_DataField = value;
				this.RaisePropertyChanged("Assign_UserBased_Security_Groups_Data");
			}
		}

		[XmlAttribute(Form = XmlSchemaForm.Qualified)]
		public string version
		{
			get
			{
				return this.versionField;
			}
			set
			{
				this.versionField = value;
				this.RaisePropertyChanged("version");
			}
		}

		protected void RaisePropertyChanged(string propertyName)
		{
			PropertyChangedEventHandler propertyChanged = this.PropertyChanged;
			if (propertyChanged != null)
			{
				propertyChanged(this, new PropertyChangedEventArgs(propertyName));
			}
		}
	}
}
