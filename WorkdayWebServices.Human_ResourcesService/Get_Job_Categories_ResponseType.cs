...
namespace WorkdayWebServices.Human_ResourcesService
{
	...
	[Serializable]
	public class Get_Job_Categories_ResponseType : INotifyPropertyChanged
	{
		private Job_CategoryObjectType[] request_ReferencesField;

		private Response_FilterType response_FilterField;

		private Response_ResultsType response_ResultsField;

		private Job_CategoryType[] response_DataField;
		...
		[XmlArray(Order = 0), XmlArrayItem("Job_Category_Reference", IsNullable = false)]
		public Job_CategoryObjectType[] Request_References
		{
			get
			{
		...
		[XmlArray(Order = 3), XmlArrayItem("Job_Category", IsNullable = false)]
		public Job_CategoryType[] Response_Data
		{
			get
			{
