...
namespace WorkdayWebServices.Resource_ManagementService
{
	...
	[Serializable]
	public class Get_Purchase_Orders_ResponseType : INotifyPropertyChanged
	{
		private Purchase_OrderObjectType[] request_ReferencesField;

		private Purchase_Order_Request_CriteriaType request_CriteriaField;

		private Response_FilterType response_FilterField;

		private Purchase_Order_Response_GroupType response_GroupField;

		private Response_ResultsType response_ResultsField;

		private Purchase_Order_OutboundType[] response_DataField;
		...
		[XmlArray(Order = 0), XmlArrayItem("Purchase_Order_Reference", IsNullable = false)]
		public Purchase_OrderObjectType[] Request_References
		{
			get
			{
		...
		[XmlElement(Order = 1)]
		public Purchase_Order_Request_CriteriaType Request_Criteria
		{
			get
			{
		...
		[XmlElement(Order = 3)]
		public Purchase_Order_Response_GroupType Response_Group
		{
			get
			{
		...
		[XmlArray(Order = 5), XmlArrayItem("Purchase_Order", IsNullable = false)]
		public Purchase_Order_OutboundType[] Response_Data
		{
			get
			{
