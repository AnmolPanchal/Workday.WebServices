...
namespace WorkdayWebServices.Student_RecordsService
{
	...
	[Serializable]
	public class Get_Transcript_Orders_ResponseType : INotifyPropertyChanged
	{
		private Transcript_OrderObjectType[] request_ReferencesField;

		private Response_FilterType response_FilterField;

		private Response_ResultsType response_ResultsField;

		private Transcript_OrderType[] response_DataField;
		...
		[XmlArray(Order = 0), XmlArrayItem("Transcript_Order_Reference", IsNullable = false)]
		public Transcript_OrderObjectType[] Request_References
		{
			get
			{
		...
		[XmlArray(Order = 3), XmlArrayItem("Transcript_Order", IsNullable = false)]
		public Transcript_OrderType[] Response_Data
		{
			get
			{
