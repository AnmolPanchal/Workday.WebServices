...
namespace WorkdayWebServices.Resource_ManagementService
{
	...
	[Serializable]
	public class Get_Expense_Rate_Table_Rules_ResponseType : INotifyPropertyChanged
	{
		private Condition_RuleObjectType[] request_ReferencesField;

		private Expense_Rate_Table_Rule_Request_CriteriaType request_CriteriaField;

		private Response_FilterType response_FilterField;

		private Expense_Rate_Table_Rule_Response_GroupType response_GroupField;

		private Response_ResultsType response_ResultsField;

		private Expense_Rate_Table_RuleType[] response_DataField;
		...
		[XmlArray(Order = 0), XmlArrayItem("Expense_Rate_Table_Rule_Reference", IsNullable = false)]
		...
		[XmlElement(Order = 1)]
		public Expense_Rate_Table_Rule_Request_CriteriaType Request_Criteria
		{
			get
			{
		...
		[XmlElement(Order = 3)]
		public Expense_Rate_Table_Rule_Response_GroupType Response_Group
		{
			get
			{
		...
		[XmlArray(Order = 5), XmlArrayItem("Expense_Rate_Table_Rule", IsNullable = false)]
		public Expense_Rate_Table_RuleType[] Response_Data
		{
			get
			{
