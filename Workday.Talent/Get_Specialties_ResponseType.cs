...
namespace WorkdayWebServices.TalentService
{
	...
	[Serializable]
	public class Get_Specialties_ResponseType : INotifyPropertyChanged
	{
		private Specialty_ParentObjectType[] request_ReferencesField;

		private Specialty_Request_CriteriaType request_CriteriaField;

		private Response_FilterType response_FilterField;

		private Response_ResultsType response_ResultsField;

		private SpecialtyType[] response_DataField;
		...
		[XmlArray(Order = 0), XmlArrayItem("Specialty_Reference", IsNullable = false)]
		public Specialty_ParentObjectType[] Request_References
		{
			get
			{
		...
		[XmlElement(Order = 1)]
		public Specialty_Request_CriteriaType Request_Criteria
		{
			get
			{
		...
		[XmlArray(Order = 4), XmlArrayItem("Specialty", IsNullable = false)]
		public SpecialtyType[] Response_Data
		{
			get
			{
